...
<div class="row">
    <div class="col-sm-5">
    <h2 style="margin-bottom:2rem;">Tabla routing IPV4</h2>
</div>
<div class="col-sm-2">
...
$json = file_get_contents('https://191.98.171.118/api/v2/monitor/router/ipv4?access_token=Nqtr71rgz98x11xmwwdd8kr4yqjf53', false, stream_context_create($arrContextOptions));
 
$data = json_decode($json, true);

// // Criterio de ordenación (en este caso, por nombre)
...
?>
  ...
  <table class="table table-hover" id="miTabla">
        ...
        <?php foreach ($data['results'] as $results): ?>
            <tr>
                <td><?= $results['ip_version'] ?></td>
                <td><?= $results['type'] ?></td>
                <td><?= $results['ip_mask'] ?></td>
                <td><?= $results['distance'] ?></td>
                <td><?= $results['gateway'] ?></td>
